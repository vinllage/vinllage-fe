    ...
    async ({ email, code }: { email: string; code: string }) => {
      ...
      if (res.ok) {
        ...
        try {
          ...
          const res = await fetchCSR(`/member/recover-acc`, {
            method: 'POST',
          ...
          // 성공 메시지 확인 후 로그인 페이지로 리다이렉트
          if (res.ok) {
            alertDialog.success(
              data.message ?? '계정 복구가 완료되었습니다.',
              () => {
                window.location.href = '/member/login'
              },
            )
          } else {
            alertDialog.error(data.message ?? '계정 복구에 실패했습니다.')
            setVerState('idle')
          }
        } catch (err: any) {
...
export default React.memo(RecoverContainer)
